...
class CacheQuerying {
	private static final Logger log = Logger.getLogger(CacheQuerying.class);
	...
	static void loadCache(String filename){
		...
		if(cache == null){
			...
			log.info("CacheQuerying - Cache Size: " + cache.size());
			log.info("");
	...
	static void makeCachePersistent(String filename){
		if(cache == null){
			log.warn("CacheQuerying - WARN: Cache is not initialized");
			return;
		}else if(cache.size()==0){
			log.warn("CacheQuerying - WARN: Cache size is 0");
			return;
		}
		...
		try {
			...
			out.close();
			fos.close();
			log.info("CacheQuerying - Cache Persisted");
		} catch (IOException ex) {
			ex.printStackTrace();
		}
	}
